...
from duration_extraction import extract_durations_with_dijkstra, extract_durations_beam
from model import Aligner
from paths import Paths
from text import Tokenizer
from utils import read_config, to_device, unpickle_binary
...
class Extractor:
    ...
    def extract_durations_for_item(self, item_file: Tuple[dict, Path, Path]) -> Tuple[dict, np.array]:
        ...
        try:
            print(f"Processing token_file: {token_file}")
            tokens = np.load(str(token_file), allow_pickle=False).astype(int)
            ...
            return None, None  # Return None to indicate an error occurred


if __name__ == '__main__':

    parser = argparse.ArgumentParser(description='Duration extraction for DeepForcedAligner.')
    parser.add_argument('--config', '-c', default='utils/aligner/aligner_train_config.yaml', type=str, help='Points to the config file.')
    ...
    args.target = str(paths.data_dir / args.target)
    ...
    for i, (item, durations) in tqdm.tqdm(enumerate(mapper), total=len(item_files)):
        if item is None or durations is None:
            continue  # Skip this item if it failed
